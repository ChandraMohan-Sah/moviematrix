...
class Movie_TestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.movie_creation_url = reverse('movie-create')  # <-- corrected

        self.moviemedia_data = {
            "name": "Nadiya Ka Paar",
            "banners": ["https://example.com/banner.jpg"],
            "thumbnails": ["https://example.com/thumb.jpg"],
            "trailers": ["https://example.com/trailer.mp4"],
            "videos": ["https://example.com/video.mp4"],
            "related_pics": ["https://example.com/related.jpg"]
        }
        self.castmedia_data = {
            "name": "Hritik Roshan",
            "profile_pics": ["https://example.com/hritik-profile.jpg"],
            "related_pics": ["https://example.com/hritik-related.jpg"]
        }
        self.creatormedia_data = {
            "name": "Radha Krishna",
            "profile_pics": ["https://example.com/krishna-profile.jpg"],
            "related_pics": ["https://example.com/krishna-related.jpg"]
        }
        self.writermedia_data = {
            "name": "ROshan the Writer",
            "profile_pics": ["https://example.com/writer-profile.jpg"],
            "related_pics": ["https://example.com/writer-related.jpg"]
        }
        self.genre_data = {"name": "Hari Bahadur"}
        self.platform_data = {"platform": "Netflix"}
    ...
    def test_create_movie(self):
        ...
        # Create Cast, Creator, Writer entries (pass media ID, not name)
        cast_res = self.client.post(self.cast_creation_url, {"castmedia": castmedia_id}, format='json')
        self.assertEqual(cast_res.status_code, status.HTTP_201_CREATED)
        cast_id = cast_res.data['id']

        creator_res = self.client.post(self.creator_creation_url, {"creatormedia": creatormedia_id}, format='json')
        self.assertEqual(creator_res.status_code, status.HTTP_201_CREATED)
        creator_id = creator_res.data['id']

        writer_res = self.client.post(self.writer_creation_url, {"writermedia": writermedia_id}, format='json')
