...
class IsMovieReviewer_OrReadOnly(permissions.BasePermission):
    """
    ...
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS:
            ...
            # print(dir(obj))
            # print(obj.user_movie_review)
            return obj.user_movie_review == request.user
    


class IsMovieVoter_OrReadOnly(permissions.BasePermission):
    """
        Custom permission to only allow the user who created the review to edit it.
        Other users can only read the review.
...
class IsUserMovieWatchlist_OrReadOnly(permissions.BasePermission):
...
class IsAdminOrUserWatchlistedMovie(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS:
...
class IsUserViewedMovie_OrReadOnly(permissions.BasePermission):
...
class IsAdminOrUserViewedMovie(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS:
            # Check permissions for read-only request
            return obj.user_viewed == request.user   # Allow read-only methods for particular user
